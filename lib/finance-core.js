...
export function normalizeEvent(row) {
  if (!row) return null;
  const staffIds = parseStaffIds(row.staff_ids);
  const staffAssigned = parseStaffIds(row.staff_assigned);
  const rawDate = String(row.date || '');
  const duration = Number(row.duration ?? (eventHoursFromTimes(row.start_time || row.startTime, row.end_time || row.endTime) || 5));
  return {
    id: String(row.id),
    title: row.title || '',
    ...
    duration,
    staffIds,
    staffNames: Array.isArray(staffAssigned) ? staffAssigned.map(String) : [],
    color: row.color || '',
    notes: row.notes || '',
  };
}
...
async function nextDocNo(pool, type) {
  ...
  const result = await pool.query(`
    SELECT COALESCE(MAX(CAST(split_part(doc_no, '-', 4) AS INTEGER)), 0) + 1 AS next_no
