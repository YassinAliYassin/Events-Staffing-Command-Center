...
You can execute the `ListInvoicesByBusiness` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useListInvoicesByBusiness(dc: DataConnect, vars: ListInvoicesByBusinessVariables, options?: useDataConnectQueryOptions<ListInvoicesByBusinessData>): UseDataConnectQueryResult<ListInvoicesByBusinessData, ListInvoicesByBusinessVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListInvoicesByBusiness(vars: ListInvoicesByBusinessVariables, options?: useDataConnectQueryOptions<ListInvoicesByBusinessData>): UseDataConnectQueryResult<ListInvoicesByBusinessData, ListInvoicesByBusinessVariables>;
```
...
```
### Return Type
Recall that calling the `ListInvoicesByBusiness` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListInvoicesByBusiness` Query is of type `ListInvoicesByBusinessData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListInvoicesByBusiness`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ListInvoicesByBusinessVariables } from '@dataconnect/generated';
import { useListInvoicesByBusiness } from '@dataconnect/generated/react'

export default function ListInvoicesByBusinessComponent() {
  // The `useListInvoicesByBusiness` Query hook requires an argument of type `ListInvoicesByBusinessVariables`:
  ...
  const query = useListInvoicesByBusiness(listInvoicesByBusinessVars);
  // Variables can be defined inline as well.
  const query = useListInvoicesByBusiness({ businessId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListInvoicesByBusiness(dataConnect, listInvoicesByBusinessVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListInvoicesByBusiness(listInvoicesByBusinessVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListInvoicesByBusiness(dataConnect, listInvoicesByBusinessVars, options);
...
export default function CreateBusinessComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ name: ..., email: ..., currency: ..., });
...
export default function CreateClientComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ name: ..., email: ..., businessId: ..., });
...
## AddLineItem
You can execute the `AddLineItem` Mutation using the `UseMutationResult` object returned by the following Mutation hook function (which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts)):
```javascript
useAddLineItem(options?: useDataConnectMutationOptions<AddLineItemData, FirebaseError, AddLineItemVariables>): UseDataConnectMutationResult<AddLineItemData, AddLineItemVariables>;
```
You can also pass in a `DataConnect` instance to the Mutation hook function.
```javascript
useAddLineItem(dc: DataConnect, options?: useDataConnectMutationOptions<AddLineItemData, FirebaseError, AddLineItemVariables>): UseDataConnectMutationResult<AddLineItemData, AddLineItemVariables>;
```
...
```
### Return Type
Recall that calling the `AddLineItem` Mutation hook function returns a `UseMutationResult` object. This object holds the state of your Mutation, including whether the Mutation is loading, has completed, or has succeeded/failed, among other things.

To check the status of a Mutation, use the `UseMutationResult.status` field. You can also check for pending / success / error status using the `UseMutationResult.isPending`, `UseMutationResult.isSuccess`, and `UseMutationResult.isError` fields.

To execute the Mutation, call `UseMutationResult.mutate()`. This function executes the Mutation, but does not return the data from the Mutation.

To access the data returned by a Mutation, use the `UseMutationResult.data` field. The data for the `AddLineItem` Mutation is of type `AddLineItemData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `AddLineItem`'s Mutation hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, AddLineItemVariables } from '@dataconnect/generated';
import { useAddLineItem } from '@dataconnect/generated/react'

export default function AddLineItemComponent() {
  // Call the Mutation hook function to get a `UseMutationResult` object which holds the state of your Mutation.
  const mutation = useAddLineItem();

  // You can also pass in a `DataConnect` instance to the Mutation hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const mutation = useAddLineItem(dataConnect);

  // You can also pass in a `useDataConnectMutationOptions` object to the Mutation hook function.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  const mutation = useAddLineItem(options);
  ...
  const mutation = useAddLineItem(dataConnect, options);

  // After calling the Mutation hook function, you must call `UseMutationResult.mutate()` to execute the Mutation.
  // The `useAddLineItem` Mutation requires an argument of type `AddLineItemVariables`:
  ...
  mutation.mutate(addLineItemVars);
  // Variables can be defined inline as well.
  mutation.mutate({ invoiceId: ..., description: ..., quantity: ..., unitPrice: ..., });

  // You can also pass in a `useDataConnectMutationOptions` object to `UseMutationResult.mutate()`.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  mutation.mutate(addLineItemVars, options);
  ...
  // If the Mutation is successful, you can access the data returned using the `UseMutationResult.data` field.
  if (mutation.isSuccess) {
    console.log(mutation.data.lineItem_insert);
  }
  return <div>Mutation execution {mutation.isSuccess ? 'successful' : 'failed'}!</div>;
}
```
